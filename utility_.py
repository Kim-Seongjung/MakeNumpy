...
def crawl_folder(folder_path): #search_str 
    search_path_list=[]
...
def pic2Numpy(source, save_path,batch_size, save_name ,img_size ,color_ch=1 , type_ = 'strict'):
    if type(source)==list:
        ...
        batch_count=0
        n_lst=len(source)
        print 'list�쓽 媛��닔 :' , n_lst
        train_img = np.zeros([batch_size , img_size[0] ,img_size[1] ,color_ch])
        print '珥� �뙆�씪�쓽 媛��닔 : ' , len(source) 
        #print list_file
        print '留뚮뱾�뼱吏� �뙆�씪�쓽 媛��닔 :' , int(len(source)/batch_size)
        print '踰꾨젮吏��뒗 �뙆�씪�쓽 媛��닔 :' , int(len(source)%batch_size)
        ...
        print '珥� �뙆�씪�쓽 媛��닔 : ' , len(list_file) 
        #print list_file
        print '留뚮뱾�뼱吏� �뙆�씪�쓽 媛��닔 :' , int(len(list_file)/batch_size)
        print '踰꾨젮吏��뒗 �뙆�씪�쓽 媛��닔 :' , int(len(list_file)%batch_size)
        train_img = np.zeros([batch_size , img_size[0] ,img_size[1] ,color_ch])    
...
def Set_ImageSize(source ):
    ...
    if type(source)==list:
        ...
        returned_np_list=[]
        max_col=0;max_row=0;
        #list�쓽 max_row �� max_col�쓣 援ы븳�떎.
        ...
        for ele in lst_dcm_array:
            enlarged_np=np.zeros([max_row , max_col]) #drawing_paper �뿬湲곕떎媛� �씠�젣 洹몃┫嫄곕떎.
...
def pic2numpy_TVT(path1 , path2 , train_rate , val_rate ,save_path ,img_row,img_col, \
    ...
    print '紐⑤뱺 �궗吏� 媛��닔 ' , n_list_path_all
    print 'training  媛쒖닔 : ' , n_train
    print 'validation  媛쒖닔 : ',n_val
    print 'test  媛쒖닔 : ',n_test
...
def pic2numpy_TVT_divide(divide_ind,path1 , path2 , train_rate , val_rate ,save_path ,img_row,img_col, color_ch,sort_type = 'random'):    
    ...
    while(n_train%divide_ind!=0):  #reduce validation and add to n_train
        n_train=n_train+1
        n_val=n_val-1
        
    print 'all number of list ' , n_list_path_all
    ...
    #define img_np 
        ...
        if len(np.shape(img_np))==2: # confirm image type whether grey or color 
            img_np = np.expand_dims(img_np,2) #if image type is grey , have to expand dim for inputting tensor
            
    #define lab_np 
    #img=mpimg.imread('stinkbug.png')
    #train_separation_ind is 
        if dic_path_lab_all[ele]==1:
            lab_np=0
        elif dic_path_lab_all[ele]==2:
            lab_np=1
...
def pic2numpy_TVT_divide_modify(divide_ind,path1 , path2 , train_rate , val_rate ,save_path ,img_row,img_col, color_ch, n_data= 'same'):    
    #problem fixed
    n_class=2
    ...
    if(n_data == 'same'):
        if(n_list_1 > n_list_2):
            list_path_1=list_path_1[:n_list_2]
        elif (n_list_2 > n_list_1):
            list_path_2= list_path_2[:n_list_1]
            
        assert len(list_path_2)== len(list_path_1)
   ...
   # np_train=np.zeros([n_train/divide_ind , img_row , img_col , color_ch])
    ...
    #train_separation_ind is 
        ...
        if train_count < (n_train/divide_ind) :
     #       np_train[train_count] =img_np
            np_train_lab[train_count,lab_np] =1 
            train_count+=1
            if train_count==(n_train/divide_ind) and train_separation_ind < divide_ind: 
      #          np.save(save_path+'/'+'train_'+str(train_separation_ind)+'_img',np_train)
                np.save(save_path+'/'+'train_'+str(train_separation_ind)+'_lab',np_train_lab)
            ...
            print 'val_count'
       #     np_val[val_count] = img_np
            np_val_lab[val_count,lab_np]=1
            val_count +=1

        elif test_count < n_test:
            print 'test_count'
        #    np_test[test_count] = img_np
            np_test_lab[test_count,lab_np] =1 
            test_count +=1 
